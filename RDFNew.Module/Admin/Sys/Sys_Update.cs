...
{
    public class Sys_Update : ISingle
    {
        const string MTABLE = "Sys_Update";
        const string MKEY = "UpdateCode";

        public object[] GetMaster(string KeyValue)
        {            
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Sys_Update.UpdateCode", "=", "UpdateCode", KeyValue));
            return GetMaster(qs);            
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Select Top {2} 
                            Sys_Update.*
                    From Sys_Update                             
                    Where 1=1 {0} And Sys_Update.UpdateCode Not In (
                        Select Top {3} Sys_Update.UpdateCode 
                        From Sys_Update 
                        Where 1=1 {0}      
                        {1}                      
                    ) 
