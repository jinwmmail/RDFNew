...
{
    public class Flow_Job : ISingle
    {
        const string MTABLE = "Flow_Job";
        const string MKEY = "JobID";

        public object[] GetMaster(string KeyValue)
        {            
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Flow_Job.JobID", "=", "JobID", KeyValue));
            return GetMaster(qs);            
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Select Top {2} 
                            Flow_Job.* 
                    From Flow_Job 
                    Where 1=1 {0} And Flow_Job.JobID Not In (
                        Select Top {3} Flow_Job.JobID 
                        From Flow_Job 
                        Where 1=1 {0}      
                        {1}                      
                    ) 
