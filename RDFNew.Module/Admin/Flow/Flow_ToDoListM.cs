...
{
    public class Flow_ToDoListM : ISingle
    {
        const string MTABLE = "Flow_ToDoListM";
        const string MKEY = "ToDoListMID";

        public object[] GetMaster(string KeyValue)
        {
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Flow_ToDoListM.ToDoListMID", "=", "ToDoListMID", KeyValue));
            return GetMaster(qs);
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Select Top {2} 
                            Flow_ToDoListM.*
                    From Flow_ToDoListM                         
                    Where 1=1 {0} And Flow_ToDoListM.ToDoListMID Not In (
                        Select Top {3} Flow_ToDoListM.ToDoListMID 
                        From Flow_ToDoListM                             
                        Where 1=1 {0}      
                        {1}                      
                    ) 
        ...
        public object[] ApplyMaster(DataTable dtM, RDFNew.Module.DALEntity.Sys_Log la)
        {
            ...
            {
                dtDetail = new RDFNew.Module.Admin.Flow.Flow_ToDoListD().GetDataByParent(
                    dtM.Rows[0]["ToDoListMID", DataRowVersion.Original].ToString())[1] as DataTable;
