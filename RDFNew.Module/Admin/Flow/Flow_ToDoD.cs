...
{
    public class Flow_ToDoD : ISingle
    {
        const string MTABLE = "Flow_ToDoD";
        const string MKEY = "ToDoDID";

        public object[] GetDataByParent(string ParentKey)
        {
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Flow_ToDoD.ToDoMID", "=", "ToDoMID", ParentKey));
            qs.OrderBy = " Flow_ToDoD.Seq ";
            return GetMaster(qs);
        }
        ...
        {
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Flow_ToDoD.ToDoDID", "=", "ToDoDID", KeyValue));
            return GetMaster(qs);
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Select Top {2} 
                            Flow_ToDoD.*
                    From Flow_ToDoD                              
                    Where 1=1 {0} And Flow_ToDoD.ToDoDID Not In (
                        Select Top {3} Flow_ToDoD.ToDoDID 
                        From Flow_ToDoD                                                         
                        Where 1=1 {0}      
                        {1}                      
                    ) 
        ...
        public object[] ApplyMaster(DataRow drM, string MID, DataTable dtD, SqlTransaction tran, RDFNew.Module.DALEntity.Sys_Log la)
        {
            ...
            foreach (DataRow dr in dtD.Rows)
            {
                if (dr.RowState == DataRowState.Added)
                {
                    ...
                    dr["ToDoMID"] = MID;
                    if (dr["Seq"].ToString() == "")
                        dr["Seq"] = DALHelper.GetDetailSeq(tran, "", "0000", 1, "Flow_ToDoD", "Seq", "ToDoMID", MID);
