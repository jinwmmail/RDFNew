...
{
    public class Bas_Material : ISingle
    {
        const string MTABLE = "Bas_Material";
        const string MKEY = "MaterialID";

        public object[] GetMaster(string KeyValue)
        {            
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Bas_Material.MaterialID", "=", "MaterialID", KeyValue));
            return GetMaster(qs);            
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Select Top {2} 
                            Bas_Material.* 
                    From Bas_Material 
                    Where 1=1 {0} And Bas_Material.MaterialID Not In (
                        Select Top {3} Bas_Material.MaterialID 
                        From Bas_Material 
                        Where 1=1 {0}      
                        {1}                      
                    ) 
