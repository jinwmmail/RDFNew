...
{
    public class Pur_PoM : ISingle
    {
        const string MTABLE = "Pur_PoM";
        const string MKEY = "PoMID";

        public object[] GetMaster(string KeyValue)
        {            
            RDFNew.Module.DALEntity.QuerySet qs = new RDFNew.Module.DALEntity.QuerySet();
            qs.QueryInfos.Add(new RDFNew.Module.DALEntity.QueryInfo("Pur_PoM.PoMID", "=", "PoMID", KeyValue));
            return GetMaster(qs);            
        }
        ...
        public object[] GetMaster(DALEntity.QuerySet qrys)
        {
            ...
            try
            {
                ...
                if (qrys == null || qrys.PageInfo == null)
                {
                    Sql = @"
                    Select 
                            Pur_PoM.*,b.PartnerName
                    From Pur_PoM 
                        Left Join Bas_Partner b On b.PartnerID=Pur_PoM.PartnerID
                    Where 1=1 {0} {1}
                    ";
                ...
                else
                {
                    ...
                    From Pur_PoM 
                        Left Join Bas_Partner b On b.PartnerID=Pur_PoM.PartnerID
                    Where 1=1 {0} And Pur_PoM.PoMID Not In (
                        Select Top {3} Pur_PoM.PoMID 
                        From Pur_PoM 
                            Left Join Bas_Partner b On b.PartnerID=Pur_PoM.PartnerID
                        Where 1=1 {0}      
                        {1}                      
        ...
        public object[] ApplyMaster(DataTable dtM, DataTable dtD, RDFNew.Module.DALEntity.Sys_Log la)
        {
            ...
            try
            {
                foreach (DataRow dr in dtM.Rows)
                {
                    ...
                    //处理子表
                    new Pur_PoD().ApplyMaster(dr, rtns[1].ToString(), dtD,tran, la);
                }
                tran.Commit();
            }
            catch (Exception ex)
