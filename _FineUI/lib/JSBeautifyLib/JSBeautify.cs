    ...
    public class JSBeautify
    {
        ...
        private string[] get_next_token(ref int parser_pos)
        {
            ...
            if (wordchar.Contains(c))
            {
                if (parser_pos < input.Length)
                {
                    while (wordchar.Contains(input[parser_pos].ToString()))
                    {
                        c += input[parser_pos];
                        parser_pos++;
                        if (parser_pos == input.Length)
                ...
                )
            {
                ...
                if (sep == "/")
                {
                    ...
                    while ((parser_pos < input.Length) && (wordchar.Contains(input[parser_pos].ToString())))
                    {
                        resulting_string += input[parser_pos];
                        parser_pos += 1;
                    }
            ...
            {
                var sharp = "#";
                if ((parser_pos < input.Length) && (digits.Contains(input[parser_pos].ToString())))
                {
                    do
                    {
                        c = input[parser_pos].ToString();
            ...
            if (((IList<string>)punct).Contains(c))
            {
                while ((parser_pos < input.Length) && (((IList<string>)punct).Contains(c + input[parser_pos])))
                {
                    c += input[parser_pos];
                    parser_pos += 1;
                    if (parser_pos >= input.Length)
            ...
            while (true)
            {
                ...
                switch (token_type)
                {
                    ...
                    case "TK_START_EXPR":
                        ...
                        else if (((IList<string>)line_starters).Contains(last_word))
                        {
                            print_space();
                        }
                        print_token();
                    ...
                    case "TK_WORD":
                        ...
                        {
                            if (!(new List<string> { "else", "catch", "finally" }).Contains(token_text.ToLower()))
                            {
                                prefix = "NEWLINE";
                            }
                            else
                        ...
                        if ((last_type != "TK_END_BLOCK") && ((new List<string> { "else", "catch", "finally" }).Contains(token_text.ToLower())))
                        {
                            print_newline(null);
                        }
                        else if ((((IList<string>)line_starters).Contains(token_text)) || (prefix == "NEWLINE"))
                        {
                            if (last_text == "else")
                            {
                            ...
                            {
                                if ((((IList<string>)line_starters).Contains(token_text)) && (last_text != ")"))
                                {
                                    print_newline(null);
                                }
                            }
