...
{
    public partial class Bas_Job : App_Com.PageSingle
    {
        ...
        protected override string AddData()
        {
            ...
            RDFNew.Module.Admin.Bas.Bas_Job obj = new RDFNew.Module.Admin.Bas.Bas_Job();
            DataTable dt = RDFNew.Module.DALHelper.GetMasterEmpty(null, "Bas_Job");
            DataRow dr;
            dr = dt.NewRow();
            ...
            object[] data = obj.ApplyMaster(dt.GetChanges(DataRowState.Added),
                                        App_Com.Helper.BuildLog("Bas_Job", "add"));
            ...
            if (data[0].ToString() == "0") //正常
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    data = obj.ApplyMaster(dt.GetChanges(DataRowState.Modified),
                                            App_Com.Helper.BuildLog("Bas_Job", "edit"));
                    if (data[0].ToString() != "0") //正常                
                        throw data[1] as Exception;
                    else
                        return data[1].ToString();
